...
// ElectricityMapsMockResponse matches the mock server
...
func (r *CarbonInfoReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if carbonInfo.Spec.OverrideIntensity > 0 {
		...
		// Query the Mock API Server
		url := fmt.Sprintf("http://localhost:8081/carbon?zone=%s", carbonInfo.Spec.Region)
		...
		// Inject Electricity maps key into the headers
		reqAPI.Header.Add("auth-token", "ELECTRICITYMAPS-SANDBOX-API-KEY-HERE")
		...
		if apiFailed {
			if carbonInfo.Spec.StaticIntensity > 0 {
				...
				// Push the error to Kubernetes and abort when there's no fallback
	...
	RegionCarbonIntensity.WithLabelValues(carbonInfo.Spec.Region).Set(float64(newIntensity)) // update region metric for Grafana

	// Update the CRD Status if it changed
	...
	// Find all nodes in this specific region
	if err := r.List(ctx, &nodeList, client.MatchingLabels{"topology.kubernetes.io/region": carbonInfo.Spec.Region}); err != nil {
		...
		// Loop through them and apply the annotation
		for _, node := range nodeList.Items {
			...
			NodeCarbonIntensity.WithLabelValues(node.Name).Set(float64(newIntensity)) // node metric for Grafana
	...
	// Requeue the loop based on the pollSeconds spec
